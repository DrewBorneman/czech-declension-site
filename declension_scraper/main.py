...
class DeclensionSpider(scrapy.Spider):
    ...
    def start_requests(self):
        ...
        url = f'https://sklonuj.cz/slovo/{encoded_noun}'
        if(self.anim):
            url += f"?inpZivotne=1"
        yield Request(
            url=url, 
    ...
    def parse(self, response):
        try:
            sing_table = response.xpath('//*[@id="mainContent"]/div[4]/div[3]/div[1]/table')
            if not sing_table:
                self.logger.error(f"Singular Table not found for noun: {self.noun}")
                with open(f"debug_{self.noun}.html", "w", encoding='utf-8') as f:
                    f.write(response.text)
                return
            pl_table = response.xpath('//*[@id="mainContent"]/div[4]/div[3]/div[2]/table')
            if not pl_table:
                self.logger.error(f"Plural Table not found for noun: {self.noun}")
                with open(f"debug_{self.noun}.html", "w", encoding='utf-8') as f:
                    f.write(response.text)
                return
            
            for i in range(0, 7):
                sing_row = sing_table.xpath('.//tbody/tr[position()]')[i]
                pl_row = pl_table.xpath('.//tbody/tr[position()]')[i]
                if sing_row and pl_row:
                    sing_form = sing_row.xpath('.//td[2]//text()').get(default='').strip()
                    pl_form = pl_row.xpath('.//td[2]//text()').get(default='').strip()
                    print(f"sing form: {str(sing_form)}")
                    print(f"pl form: {str(pl_form)}")
                    self.declension.set_declension(i, sing_form, pl_form)
                else:
                    self.logger.error(f"Row {i} not found for noun: {self.noun}")
...
class DeclensionScraper:
    def __init__(self):
        ...
        self.nouns_to_process: List[dict] = []
    ...
    def process_next_noun(self):
        ...
        if self.current_noun_index < len(self.nouns_to_process):
            ...
            self.process.crawl(DeclensionSpider, noun=noun['word'], an=noun['an'], rank=self.current_noun_index)

    def scrape_nouns(self, nouns: List[dict]):
