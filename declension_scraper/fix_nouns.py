...
def main():
    ...
    nouns_json = sys.argv[1]
    an_json = sys.argv[2]
    scraper = DeclensionScraper()
    
    try:
        with open(an_json, 'r', encoding='utf-8') as an_file:
            an_data = json.load(an_file)
        
        if 'an' not in an_data:
            print("Error: JSON file must contain a 'nouns' array")
            sys.exit(1)

        with open(nouns_json, 'r', encoding='utf-8') as nouns_file:
            nouns_data = json.load(nouns_file)
        
        if 'nouns' not in nouns_data:
            print("Error: JSON file must contain a 'nouns' array")
            sys.exit(1)
        
        for i,noun in enumerate(nouns_data['nouns']):
            # dump the noun
            print(f"Processing noun: {noun['word']}")
            if noun['word'] in an_data['an']:
                print(f"found {str(noun['word'])}")
                nouns_data['nouns'][i]['an'] = True

        #dump the updated nouns data
        with open(nouns_json, 'w', encoding='utf-8') as nouns_file:
            json.dump(nouns_data, nouns_file, ensure_ascii=False, indent=2)
        ...
        print(f"Error: File '{nouns_json}' not found")
        sys.exit(1)
    except json.JSONDecodeError:
        print(f"Error: '{nouns_json}' is not a valid JSON file")
        sys.exit(1)
    except Exception as e:
